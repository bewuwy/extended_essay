...
\begin{document}
    ...
    \section{Finding roads for different polygonal wheels}
        ...
        However, the magnitude of the slope has to be bigger than $1$, as by decreasing the slope, the angle ``on top of the road'' will increase. (The other value gives an angle equal to $120\degree$ instead of $60\degree$) Thus, the correct result is:
        \begin{align*}
            |m| = \sqrt{3}
        \end{align*}
        ...
        Moreover, a triangular shape is not a good pick for a bridge, as when it would be rotated by $180\degree$ so that ships could swim beneath it, it would stand on its vertex. Hence, this solution would be very dangerous in real life due to its instability (due to the steep road, big friction force would also be needed so that the bridge would not slip while rolling). This is actually the case for all odd-sided polygons, so it is perhaps more interesting to look at an even-sided polygon~-~more precisely a hexagon.
        ...
        However, now we want $|m_1|<1$, as the angle ``on top of the catenary'' should be obtuse. Thus:
        \begin{align}
            |m| = \frac{\sqrt{3}}{3} 
        \end{align}
        ...
        The road, along with the hexagon of side length $\frac{2\sqrt{3}}{3}$ (as the distance from the centre of the polygon to the road had to be equal to $1$) can be seen graphed below in Figure~\ref{fig:hexagon}:
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.8\linewidth]{images/hexagon_bridge.png}
            \caption{Hexagon with its road}\label{fig:hexagon}
        \end{figure}

        It is interesting to note, that in this case there have to be 4 catenaries in order for the polygon to rotate by $180\degree$. The square needed only 3, while the triangle only 2 (or actually 2 and a half~-~to keep it balanced on the vertex when the bridge is open). However, the more of the catenaries, the shorter they are, so the road becomes shorter horizontally. As the shape will approach a circle, the length of the road to rotate by half a revolution will approach $\pi$.

        Another observation that can be made is that the more sides the polygon has, the sooner the catenary is truncated and the less steep it becomes. This is quite intuitive, because as the number of sides of a polygon increase, the more circular it becomes, so the road is more and more like the one ideal for a circle~-~a straight line.
    ...
    \section{Conclusion}
        ...
        When designing perfect roads for non-circular shapes, the catenaries become very useful, irrespective of the polygon that the ``wheel'' is. However, when we try to apply those concepts of rolling polygons in the real world, like the architects of Cody Dock bridge did, some adjustments have to be made.

        Ironically, to make the non-circular rolling bridges a reality, circles have to be added to them. Thanks to those rounded corners the pressure at the vertices can be decreased, but it is all at the cost of catenaries. The roads required for circles, which have their mass centres outside of them, are way more complicated than simple catenaries. Hence, in the real world, when physics come into play, the catenaries seem to be not enough to construct such innovative bridges. More sophisticated integration methods (or simply approximations) are also required to find the not-easy road fragments.

        Even though the innovations that the catenaries bring into the world of non-circular wheels are not very practical, they show that Mathematics as a piece of knowledge, that is very interconnected and that those connections appear even where we least expect them. Moreover, using them in such unusual ways can be great for artists or engineers (as in the Cody Dock case) trying to convey some message. This is further exemplified by the fact, that the Cody Dock bridge won the \textit{2023 Bridges Design Award}~\cite{bridges_awards}. Overall, the use of catenaries is needed when constructing non-circular, ``rolling'' bridges and perhaps in the future many more seemingly non-practical structures.
